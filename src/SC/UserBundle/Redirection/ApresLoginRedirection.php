...
class ApresLoginRedirection implements AuthenticationSuccessHandlerInterface
{
    ...
    public function onAuthenticationSuccess(Request $request, TokenInterface $token)
    {
        ...
        if (in_array('ROLE_ADMIN', $rolesTab, true))
            $redirection = new RedirectResponse($this->router->generate('sc_orange_admin_index'));
        // sinon, s'il s'agit d'un commercial on le redirige vers le CRM
        elseif (in_array('ROLE_SUPER_ADMIN', $rolesTab, true))
            $redirection = new RedirectResponse($this->router->generate('sc_orange_superadmin_index'));
        // sinon il s'agit d'un membre
        else
            $redirection = new RedirectResponse($this->router->generate('sc_orange_homepage'));
